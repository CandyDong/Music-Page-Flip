...
repeat_paths = {
    "start": ["resources/template/repeat-line.png"],
    "1": ["resources/template/repeat-1.png"],
    "2": ["resources/template/repeat-2.png"]
}
...
# Repeats
repeat_start_imgs = [cv2.imread(repeat_start, 0) for repeat_start in repeat_paths['start']]
repeat_1_imgs = [cv2.imread(repeat_start, 0) for repeat_start in repeat_paths['1']]
repeat_2_imgs = [cv2.imread(repeat_start, 0) for repeat_start in repeat_paths['2']]
...
# Repeats
repeat_lower, repeat_upper, repeat_thresh = 50, 150, 0.85
...
if __name__ == "__main__":
    ...
    for i in range(len(staffs)):
        ...
        repeat_start_boxes = locate_templates(staff_img, repeat_start_imgs, repeat_lower, repeat_upper, repeat_thresh)
        repeat_start_boxes = merge_boxes([j for i in repeat_start_boxes for j in i], 0.5)

        print("[INFO] Displaying Matching Results on staff", i + 1)
        for box in repeat_start_boxes:
            box.draw(staff_img_color, red, box_thickness)
            text = "repeat"
            font = cv2.FONT_HERSHEY_DUPLEX
            textsize = cv2.getTextSize(text, font, fontScale=0.7, thickness=1)[0]
            ...
            y = int(box.getCorner()[1] + box.getHeight() + 20)
            cv2.putText(staff_img_color, text, (x, y), font, fontScale=0.7, color=red, thickness=1)
            file.write("start,%d,%d\n" % (x, y))
            line = Primitive("repeat", 0, box)
            staff_primitives.append(line)

        repeat_1_boxes = locate_templates(staff_img, repeat_1_imgs, repeat_lower, repeat_upper, repeat_thresh)
        repeat_1_boxes = merge_boxes([j for i in repeat_1_boxes for j in i], 0.5)

        print("[INFO] Displaying Matching Results on staff", i + 1)
        for box in repeat_1_boxes:
            box.draw(staff_img_color, red, box_thickness)
            text = "1"
            font = cv2.FONT_HERSHEY_DUPLEX
            textsize = cv2.getTextSize(text, font, fontScale=0.7, thickness=1)[0]
            ...
            y = int(box.getCorner()[1] + box.getHeight() + 20)
            cv2.putText(staff_img_color, text, (x, y), font, fontScale=0.7, color=red, thickness=1)
            file.write("repeat_1,%d,%d\n" % (x, y))
            line = Primitive("1", 0, box)
            staff_primitives.append(line)

        repeat_2_boxes = locate_templates(staff_img, repeat_2_imgs, repeat_lower, repeat_upper, repeat_thresh)
        repeat_2_boxes = merge_boxes([j for i in repeat_2_boxes for j in i], 0.5)

        print("[INFO] Displaying Matching Results on staff", i + 1)
        for box in repeat_2_boxes:
            box.draw(staff_img_color, red, box_thickness)
            text = "2"
            font = cv2.FONT_HERSHEY_DUPLEX
            textsize = cv2.getTextSize(text, font, fontScale=0.7, thickness=1)[0]
            ...
            y = int(box.getCorner()[1] + box.getHeight() + 20)
            cv2.putText(staff_img_color, text, (x, y), font, fontScale=0.7, color=red, thickness=1)
            file.write("repeat_2,%d,%d\n" % (x, y))
            line = Primitive("2", 0, box)
            staff_primitives.append(line)
